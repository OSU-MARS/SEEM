...
namespace Osu.Cof.Ferm.Species
{
    internal class DouglasFir
    {
        ...
        /// </remarks>
        public static float BrucePsmeAbgrGrowthEffectiveAge(float siteIndexFromGround, float treeHeight, float GP, out float potentialHeightGrowth)
        {
            float X1 = 13.25F - siteIndexFromGround / 20.0F;
            float X2 = 63.25F - siteIndexFromGround / 20.0F;
            float X3 = siteIndexFromGround / 100.0F;
            float B2 = -0.447762F - 0.894427F * X3 + 0.793548F * X3 * X3 - 0.171666F * X3 * X3 * X3;
            float X1toB2 = MathV.Pow(X2, B2);
            float B1 = MathV.Ln(4.5F / siteIndexFromGround) / (MathV.Pow(X1, B2) - X1toB2);
            float XX1 = MathV.Ln(treeHeight / siteIndexFromGround) / B1 + X1toB2;
